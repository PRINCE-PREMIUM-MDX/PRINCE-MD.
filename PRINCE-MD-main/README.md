...
├── prince-md/             # Plugins (15 included by default)
├── public/
...
- **`prince/`** is the engine — the parts every install needs regardless of which plugins are enabled: pairing/session handling, the plugin loader, group-related helpers, and message utilities.
- **`prince-md/`** is where features live. Every file in here is a self-contained plugin. Drop a new one in, and it's picked up automatically — no need to touch the engine.
...
1. **Clone your fork**
   ...
   ```bash
   git clone https://github.com/PRINCE-PREMIUM-MDX/PRINCE-MD.git
   cd PRINCE-MD
   ```
...
3. **Configure your environment**
   ...
   | `OWNER_NUMBER`   | No       | +243860885022 (digits only), used for owner-only commands.      |
   | `NEWSLETTER_JID` | No       | A WhatsApp channel JID to auto-react to, if you use that feature.      |
   | `CHANNEL_LINK`   | No       | https://whatsapp.com/channel/0029Vb8KrLcJpe8piGeSfH0i.                                      |
   | `PM2_NAME`       | No       | PRINCE-XMD .        |
...
PRINCE-MD ships with 15 plugins in `PRINCE/`, covering the essentials: session/group protection, media conversion, downloaders, and search. This keeps the default install lean — add more plugins as you need them.
...
Drop a new file into `Prince/`. It's picked up automatically the next time the plugin loader runs (on boot, and again a moment after any file change while the bot is running).
...
The `handler` receives a context object with everything you'll typically need: `socket`, `msg`, `sender`, `command`, `args`, `reply`, `isGroup`, `isOwner`, `senderNumber`, `botNumber`, and more. For group-related checks (is this user an admin, is the bot an admin), use the shared helpers in `prince/group.js`:
...
PRINCE-MD runs anywhere Node.js and outbound network access are available: a VPS, Render, Railway, Fly.io, or similar. Platforms that read `apt.txt` will install `ffmpeg` automatically; otherwise install it yourself on the host.
