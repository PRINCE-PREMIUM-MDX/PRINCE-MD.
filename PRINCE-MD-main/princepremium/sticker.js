...
/**
 * ⚡ Media to Sticker Generator (Prince-Mini System)
 */
module.exports = {
    name: "sticker_maker",
    category: "media",
    ...
    handler: async ({ socket, msg, sender, command, args }) => {
        try {
            console.log(`[PRINCE-MINI BOT] .${command} command execution started.`);
            ...
            console.log(`[PRINCE-MINI BOT] Downloading ${mediaType} from WhatsApp...`);
            ...
            console.log("[PRINCE-MINI BOT] Converting to sticker...");
            const sticker = new Sticker(buffer, {
                pack: 'PRINCE-MD', // Your sticker pack name
                author: 'PRINCE PREMIUM', // Your name
            ...
            console.log("PRINCE-MINI BOT] Sending sticker to user...");
            await socket.sendMessage(sender, { sticker: stickerBuffer }, { quoted: msg });
            await socket.sendMessage(sender, { react: { text: "✅", key: msg.key } });

        } catch (error) {
            console.error("[PRINCE-MINI BOT] STICKER ERROR OCCURRED:", error);
            await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } });
